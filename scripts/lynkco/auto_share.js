...
const SCRIPT_VERSION = "2026-09-07-v1";
...
async function doDailySign(token) {
  ...
  if (signSuccess) {
    ...
    notify("领克签到成功", tip, "获得能量:" + (data.rewardEnergyNumber || 0));
    pushSummary.signStatus = "✔ 签到成功";
    pushSummary.signDetail = tip;
    return true;
  }

  // 4. 签到接口网关拦截时，再次核验日历状态
  const checkRet = await apiCall("GET", EP_SIGN_DAY_INFO, token);
  const checkObj = checkRet.obj || {};
  if (checkRet.status === 200 && checkObj.data && Number(checkObj.data.signStatus) === 1) {
    pushSummary.signStatus = "✔ 今日已签到";
    pushSummary.signDetail = "已确认为已签到状态";
    return true;
  }

  log("Daily sign failed, status=" + ret.status);
  pushSummary.signStatus = ret.status === 403 ? "⚠️ 网关限制原生调用 (需在App点签到)" : "❌ 签到未成功";
  pushSummary.signDetail = ret.status === 403 ? "HTTP 403 (不影响后续文章分享得积分)" : (obj.message || "失败");
  return false;
}
...
function handleHttpRequest() {
  ...
  if (token) {
    ...
    write(formatGmt8Day(Date.now()), STORE_ACCESS_TOKEN_DAY);
    if (token !== oldToken) {
      log("打开 App 捕获到新 Token: " + mask(token, 12, 8));
      updated = true;
    }
  }

  if (svcsid) write(svcsid, STORE_SVCSID);
  if (deviceInfo.deviceId) write(deviceInfo.deviceId, STORE_DEVICE_ID);
  if (deviceInfo.glDevId) write(deviceInfo.glDevId, STORE_GL_DEV_ID);

  if (refreshToken) {
    write(refreshToken, STORE_REFRESH_TOKEN);
    log("捕获到 refreshToken: " + mask(refreshToken, 12, 8));
    notify("领克长效凭证已更新", "已保存 refreshToken", "后续将全自动续期并执行任务");
  } else if (updated) {
    notify("领克 Token 已捕获", "打开 App 自动更新成功", "Token 活跃就绪，Cron 将自动执行任务");
  }
...
function handleHttpResponse() {
  ...
  if (deviceInfo.deviceId) {
    ...
    log("deviceId saved from response = " + mask(deviceInfo.deviceId, 8, 6));
  }

  if (deviceInfo.glDevId) {
    write(deviceInfo.glDevId, STORE_GL_DEV_ID);
    log("gl_dev_id saved from response = " + mask(deviceInfo.glDevId, 8, 6));
  }

  const refreshTokenFromUrl = query.refreshToken || query.refreshtoken || "";
  ...
  if (refreshToken) {
    ...
    notify("领克凭证已更新", "已保存长效 refreshToken", "后续 Cron 可全自动续期并执行");
  }
  ...
  // 关键：安全透传原始响应体，绝不影响 App 本身的登录/退出等业务
...
async function runCron() {
  ...
  try {
    ...
    // 若续期未成功，回退使用打开 App 时捕获的活跃 Token
    if (!accessToken) {
      ...
      if (capturedToken) {
        log("使用打开 App 捕获的活跃 Token (捕获于 " + ageMinutes + " 分钟前): " + mask(capturedToken, 12, 8));
        accessToken = capturedToken;
        pushSummary.tokenStatus = "⚡ 活跃Token (打开App捕获, " + ageMinutes + "分钟前)";
      }
    }

    if (!accessToken) {
      pushSummary.tokenStatus = "❌ 凭证缺失或已失效";
      notify(
        "领克任务失败",
        "Token 已失效",
        "请打开领克 App，脚本将自动捕获最新 Token 并执行任务"
      );
      await sendPushPlus("领克每日任务诊断", buildReportHtml());
      $done({});
      return;
    }
